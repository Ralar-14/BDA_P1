...
# Define the schema for the top_artists dataset
top_artists_schema = StructType([
...
def visualize_top_artists(spark: SparkSession, top_artists_path: str, output_path: str, limit: int = 20):
    ...
    try:
        # Read the top artists data with the defined schema
        top_artists_df = spark.read.option("header", "true").schema(top_artists_schema).csv(top_artists_path)
...
def visualize_country_popularity(spark: SparkSession, country_popularity_path: str, output_path: str, limit: int = 15):
    ...
    try:
        # Read the country popularity data - changed from parquet to csv
        country_popularity_df = spark.read.option("header", "true").option("inferSchema", "true").csv(country_popularity_path)
...
def visualize_audio_features_correlation(spark: SparkSession, feature_correlations_path: str, output_path: str):
    ...
    try:
        # Read the feature correlations data - changed from parquet to csv
        correlations_df = spark.read.option("header", "true").option("inferSchema", "true").csv(feature_correlations_path)
